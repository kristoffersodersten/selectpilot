...
This installs dependencies, builds the extension, installs/starts Ollama when needed, selects a hardware-safe local profile, pulls and prewarms the exact generation model, installs the LaunchAgent, and refuses to finish unless the bridge and both configured models are healthy. Structured generation uses a fixed zero-temperature sampling contract and seed so repeated requests follow the same decision path.
