...
const orderIdInputEl = $('#order-id') as HTMLInputElement | null;
const syncOrderButtonEl = $('#btn-sync-order') as HTMLButtonElement | null;
...
async function doSyncOrderToken() {
  ...
  if (!orderRes.ok) {
    if (orderRes.status === 404) throw new Error('No payment detected for this order ID');
    throw new Error(`Order lookup failed (${orderRes.status})`);
  }
  const order = await orderRes.json() as { paid?: boolean; token?: string; confirmations?: number; confirmations_required?: number };
...
function bindActions() {
  ...
  selectionCardEl?.addEventListener('click', (event) => {
    ...
    if (target?.id === 'btn-first-run-example') void wrap(() => doFirstRunExample())();
  });
