import type {
  ...
  IntentClarification,
  OperationFamily,
} from '../../shared/types/intent.js';
import { resolveIntent } from './resolveIntent.js';
import {
  computeAmbiguityScore,
  needsIntentClarification,
  scoreIntentOperations,
  selectTopOperationFamily,
} from './ambiguity.js';

function inferOutputFormat(text: string, family: OperationFamily): IntentConstraints['output_format'] {
  const normalized = text.toLowerCase();
  if (normalized.includes('json') || normalized.includes('schema')) return 'json';
  if (family === 'extract' || family === 'classify') return 'structured';
  if (family === 'unknown') return 'unknown';
  return 'freeform';
}

function inferStrictness(text: string): IntentConstraints['strictness'] {
  const normalized = text.toLowerCase();
  if (normalized.includes('strict') || normalized.includes('exact') || normalized.includes('must')) return 'high';
  if (normalized.includes('rough') || normalized.includes('quick')) return 'low';
  return 'medium';
}

function inferLength(text: string): IntentConstraints['length'] {
  const normalized = text.toLowerCase();
  if (normalized.includes('one line') || normalized.includes('brief') || normalized.includes('short')) return 'short';
  if (normalized.includes('detailed') || normalized.includes('long')) return 'long';
  return 'medium';
}

function inferAction(family: OperationFamily): string {
  switch (family) {
    case 'extract':
...
function inferTarget(text: string): string | null {
  const normalized = text.toLowerCase();
  if (normalized.includes('selection')) return 'selection';
  if (normalized.includes('page')) return 'page_text';
  if (normalized.includes('article')) return 'article';
  if (normalized.includes('email')) return 'email';
  return null;
}

function buildClarification(compiledIntent: CompiledIntent): IntentClarification | null {
  if (!compiledIntent.needs_clarification) return null;
  return {
...
export function compileIntent(intentRaw: string): IntentCompilationResult {
