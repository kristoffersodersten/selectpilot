...
type RollbackResult = {
  ...
  reason: string;
  policy: RuntimeModelPolicy | null;
  audit_events_appended: number;
};
...
function appendAuditEvent(record: PromotionAuditRecord): number {
  const audit = readJson<{ events?: PromotionAuditRecord[] }>(auditPath) || {};
