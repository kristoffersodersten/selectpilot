...
export function selectRuntimeModel(
  ...
  const isSelectable = (modelId: string, allowQuarantined = false) => {
    ...
    if (!registryById.has(modelId)) return false;
    if (!allowQuarantined && quarantined.has(modelId)) return false;
    return true;
  };
  ...
  const tuple = (policy.defaults || []).find((d) =>
    d.task_family === input.taskFamily &&
